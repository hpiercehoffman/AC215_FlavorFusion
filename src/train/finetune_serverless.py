...
def download_data(local_folder):
    ...
    blobs = bucket.list_blobs(prefix="labelled_data_new/")
    for blob in blobs:
        print("Data file:", blob.name)
        if not blob.name.endswith("labelled_data_new/"):
...
def clean_data(examples):
    """Perform data cleaning by extracting reviews and annotated summaries from JSON files
    produced in the Label Studio format.
    """
    
    new_text = []
    new_summaries = []
    for i in range(len(examples['task'])):
        new_text.append(examples['task'][i]['data']['text'])
        new_summaries.append(examples['result'][i][0]['value']['text'][0])
    return {'review_str': new_text, 
...
def main(args):
    ...
    # Clean data to remove unwanted information
    raw_dataset = raw_dataset.map(clean_data,
                                remove_columns=column_names,
                                batched=True,
                                num_proc=args.num_processes,
                                desc="Cleaning input data structure")
...
if __name__ == "__main__":
    ...
    # General args
    parser.add_argument('--input_dir', type=str, help='Path to folder containing Google review json files')
    parser.add_argument('--model_output_path', type=str, help='Path to model output')
    parser.add_argument('--download', action="store_true", help="Download processed Google review data from GCS bucket")
    ...
    parser.add_argument('--k_top_longest', type=int, default=5, help='Keep only the k longest reviews for each data point')

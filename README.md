...
Pruning is implemented as a training option in our [training script](https://github.com/hpiercehoffman/AC215_FlavorFusion/blob/milestone4/src/train/train_serverless.py). For full details of how to run this script to prune a previously trained model, see [Setup Notes](https://github.com/hpiercehoffman/AC215_FlavorFusion/blob/milestone4/README.md#setup-notes).

To benchmark the results of pruning, we pruned 50% of weights in fully connected and output layers. We then calculated compressed model size, inference time, and accuracy metrics on a Nvidia T4 GPU. All of our benchmarking code can be found in the [Pruning Evaluation](https://github.com/hpiercehoffman/AC215_FlavorFusion/blob/milestone4/notebooks/pruning_evaluation.ipynb) notebook. 

The below table summarizes the results of benchmarking.

| Model                       | Memory zipped (GB) | ROUGE-1 | ROUGE-2 | ROUGE-L | Inference time (s) |
|-----------------------------|:--------------------:|:---------:|:---------:|:---------:|:--------------------------:|
| PRIMERA-multinews (zero-shot)       | 1.66               | 20.99   | 2.50  | 13.2   | 4.4              |
| PRIMERA-multinews  trained on LSARS        | 1.66               | 36.97   | 13.71   | 26.92   | 3.2               |   
| PRIMERA-multimews  trained on LSARS + pruned (50%) | 1.49               | 32.43   | 9.49    | 22.83   | 3.1       |  

Although the pruned model has a smaller compressed size, the improvement in inference time is very small. There is also a significant drop in the [ROUGE metrics](https://medium.com/nlplanet/two-minutes-nlp-learn-the-rouge-metric-by-examples-f179cc285499), which measure the linguistic similarity between the target summary and the generated summary.

Looking at the summaries generated by the pruned model, we can see that the summaries make less sense and are less relevant to the input reviews. We show an example below, where the three models were used to generate summaries for a sample set of reviews:

![image](https://github.com/hpiercehoffman/AC215_FlavorFusion/blob/milestone4/images/pruning_results_example.png)

The pruned model's summary is off-topic and doesn't mention food, while the unpruned model specifically discusses the qualities of a restaurant. Based on these results and the benchmarks above, we have decided to use the unpruned model for the next step of few-shot fine-tuning.
...
To use the `train_serverless.py` script to prune a previously trained model, you can use the `--prune` flag and specify the WandB URI of the model you would like to prune. The WandB URI should be specified via the `--wandb_download_folder` flag. The `--prune_amount` flag specifies what proportion of parameters should be pruned (between 0.0 and 1.0). 

If the training script is run in pruning mode, no actual training will take place. The desired model will be downloaded to the training container and pruned to the specified amount. Then the pruned model will be uploaded to WandB as an artifact in a new run. 
